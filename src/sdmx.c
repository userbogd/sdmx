...
const uint8_t addr_inst[] = { 0xA5, 0x5A, 0xD2, 0x5A };

const uint8_t param_inst[] = { 0xC5, 0x3A, 0x4B, 0x5A };
const uint8_t param_data[] = { 0x80, 0x00, 0x00, 0x00, 0x01, 0xC7, 0x20, 0x4B, 0x5A };

const uint8_t autoaddr_inst[] = { 0xA3, 0x96, 0xD2, 0x5A };
const uint8_t autoaddr_data[] = { 0x00, 0xD2, 0x5A };

const uint8_t curr_inst[] = { 0xAC, 0x53, 0xD2, 0x5A };
const uint8_t curr_data[] = { 0x1E, 0x1E, 0x1E, 0x1E, 0xD2, 0x5A };

uint8_t inst[4];
uint8_t data[9];
uint8_t len;

const uint8_t bit_reverse_table[256] = {  };

static void change_order(uint8_t *dat, uint8_t len)
{
    for (uint8_t i = 0; i < len; i++)
        dat[i] = bit_reverse_table[dat[i]];
}
...
IRAM_ATTR static void uart_tx_task(void *arg)
{
    ...
    while (1)
    {
        if (dmx->cfg.dmx_mode == normal)
        {
            xEventGroupWaitBits(dmx->dmx_events_group, SEND_DMX_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
        ...
        else if (dmx->cfg.dmx_mode == progr)
        {
            xEventGroupWaitBits(dmx->dmx_events_group, PROG_DMX_BIT, pdTRUE, pdFALSE, portMAX_DELAY);
            ...
            esp_rom_delay_us(45);
            /*
            uart_write_bytes(dmx->cfg.uart, autoaddr_inst, 4);
            uart_wait_tx_done(dmx->cfg.uart, portMAX_DELAY);
            esp_rom_delay_us(45);
            uart_write_bytes(dmx->cfg.uart, autoaddr_data, 3);
            */
            // Params

            memcpy(inst, param_inst, 4);
            change_order(inst, 4);
            memcpy(data, param_data, 9);
            change_order(data, 9);
            ...
            /*
            uart_write_bytes(dmx->cfg.uart, addr_inst, 4);
            uart_wait_tx_done(dmx->cfg.uart, portMAX_DELAY);
            esp_rom_delay_us(45);
            uint8_t dat[4];
            for (int i = 0; i < 4; i++)
            {
                ...
                dat[2] = 0xD2;
                dat[3] = 0x5A;
                uart_write_bytes(dmx->cfg.uart, dat, 4);
                uart_wait_tx_done(dmx->cfg.uart, portMAX_DELAY);
                esp_rom_delay_us(45);
            }*/
        }
...
const uart_config_t uart_config
    = { .baud_rate = DMX_BAUD, .data_bits = UART_DATA_8_BITS, .parity = UART_PARITY_DISABLE, .stop_bits = UART_STOP_BITS_2, .rx_flow_ctrl_thresh = 0, .flow_ctrl = UART_HW_FLOWCTRL_DISABLE };

esp_err_t InitDMXchannel(sdmx_handle_t *dmx, sdmx_config_t *cfg)
{
    memcpy(&dmx->cfg, cfg, sizeof(sdmx_config_t));

    ESP_ERROR_CHECK(uart_driver_install(cfg->uart, cfg->circ_buff_size * 2, cfg->circ_buff_size * 2, 20, &dmx->dmx_rx_queue, 0));
